...
data={"author": name_data,
                        ...
                        "subjectivity": subjectivity_data,
                        "sentiment": sentiment_data}
twitterdata=pd.DataFrame(data)
print(twitterdata)
class NewsHeadlineListener:

    def __init__(self, url=None, frequency=0):
        self.url = url
        self.headlines = []
        self.followedlinks = []
        ...
        while True:
            ...
            for htext, htext_url in new_headlines:
                if htext not in self.headlines:
                    ...
                    datenews_data.append(datenow)
                    locationnews_data.append(htext_url)
                    messagenews_data.append(htext)
                    polaritynews_data.append(polarity)
                    subjectivitynews_data.append(subjectivity)
                    sentimentnews_data.append(sentiment)
                    print('working?')
                    if len(messagenews_data) > 0:
                        newdata={'date':datenews_data,'location':locationnews_data,'message':messagenews_data,'polarity':polaritynews_data,'subjectivity':subjectivitynews_data,
                                 'sentiment':sentimentnews_data}
                        newsdataframe=pd.DataFrame(newdata)
                        print(newsdataframe)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--frequency", metavar="FREQUENCY", default=0, type=int,
                        help="How often in seconds to retrieve news headlines (default: 0 sec)")
    ...
    if args.newsheadlines:
        ...
        try:
            ...
            databank = {"author": name_data,
                        ...
                        "sentiment": sentiment_data}
            twitterdata = pd.DataFrame(databank)
            #twitterdata.to_csv(r'C:\Users\liwen\Downloads\twitterdata.csv')
            print(twitterdata)
            stream.disconnect()
            sys.exit(0)
